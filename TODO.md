...
- [x] **Tab Producción**: gráfico de serie temporal (Recharts) + tabla de distribución por ciudad — conectado a `/produccion` real. **Sub-tab "Mapa" agregado 2026-07-04** (`?vista=mapa` dentro de `/produccion`, preserva los demás filtros): mapa dinámico de sectores productivos con MapLibre GL, toggle Topográfico (OpenTopoMap, relieve+curvas de nivel)/Satelital (ESRI World Imagery) — el IGN (`ign.gob.ar`, `wms.ign.gob.ar`) no es alcanzable desde este entorno (timeout de red confirmado, con curl directo también), se usaron estas 2 fuentes gratuitas sin API key en su lugar. Superpone límites de superficie cultivada por municipio (82 polígonos reales, capa `view_superficie_por_municipios` ya cargada) + burbujas de producción por ciudad y año (6 ciudades puntuales, mismas coordenadas que `etl_nasa_power.py`; "Otros" queda afuera, no tiene ubicación puntual)
