...
- [ ] **Hallazgo real 2026-07-11, mismo patrón que la auditoría de Competencia (Fase 8)**: `ym.dataset_principal.produccion_kg` (de `dataset_principal.csv`) NO tiene desglose mensual real — es una curva sintética que se repite idéntica todos los años (verificado en 2011, 2019 y 2024): Ene=Feb=Mar=Sep siempre iguales, May=Jul siempre iguales, Oct=Nov=Dic siempre en 0 exacto. El **total anual sí es real** (calza casi exacto con el total real de `ym.inym_hoja_verde_zona` — ver más abajo), solo el reparto entre los 12 meses es fabricado. Se descubrió al construir el mapa de calor de Producción (Fase 6): una grilla año×mes hace obvio un patrón que un gráfico de línea disimula. `consumo_interno_kg`/`exportaciones_kg`/`valor_fob_usd` de la misma tabla probablemente comparten el problema (se ven pocos valores distintos repetidos por año en el CSV crudo) pero no se auditó con el mismo rigor — pendiente si se van a usar para algo mensual. **Mitigación aplicada**: el mapa de calor de Producción usa `ym.inym_hoja_verde_zona` (zona TOTAL, "ingreso de hoja verde a secadero", scrapeado real de PDFs del INYM en Fase 3c) en vez de `produccion_kg` — dato mensual genuino, con huecos reales marcados "s/d" donde no hay publicación, no inventado
