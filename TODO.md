...
- [x] **Tab Producción**: gráfico de serie temporal (Recharts) + tabla de distribución por ciudad — conectado a `/produccion` real. **Sub-tab "Mapa" agregado 2026-07-04, rediseñado 2026-07-05** (`?vista=mapa` dentro de `/produccion`): la primera versión (satelital/topo + burbujas de producción + límites) no convenció al usuario ("no me gusta"). Se ofrecieron 4 alternativas y se implementaron las 2 elegidas: **(1) Coroplético** — 19 departamentos de Misiones/Corrientes coloreados por % de superficie cultivada con yerba mate (`view_superficie_por_departamentos`, rango real 1,0%-17,8%), con leyenda; **(2) Clústeres de secaderos** — las 203 plantas de secado (`view_mat_gis_marketing_puntos_secaderos`) agrupadas por zoom con MapLibre clustering nativo, click en cluster hace zoom-in, click en punto individual muestra popup. Ambas sobre el mismo toggle Topográfico/Satelital (OpenTopoMap/ESRI World Imagery, el IGN sigue inalcanzable). El enfoque anterior (burbujas por ciudad + límites municipio + radios censales) se sacó de este mapa, no se descartó el código de radios censales en sí (sigue disponible en Mapa GIS general)
