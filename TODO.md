...
- [x] **ETLs de comercio exterior INDEC completos 2026-07-12** (ver `docs/fuentes_exportaciones_indec.md`): `etl_indec_comex_exportaciones.py` (6.094 filas, 2002-2026) y `etl_indec_comex_importaciones.py` (340 filas, 2002-2026, 0% confidencial) contra `comexbe.indec.gob.ar`, lógica compartida en `lib_indec_comex.py`. Reemplazan `ym.exportaciones` y `ym.importaciones` como fuente de verdad (`ym.exportaciones_indec`/`ym.importaciones_indec`, migraciones 004/005). `/exportaciones` e `/importaciones` reescritos con chart mensual, distribución por país y (solo exportaciones) mapa de flujos reales. Con esto **no queda ningún dato fabricado sin reemplazar en el proyecto**
- [x] **Categoría B cerrada 2026-07-12** (`docs/auditoria_datos.md` §7 punto 10): `precio_usd_kg`/`valor_fob_usd` confirmados como derivados (`valor_fob_usd / exportaciones_kg`, coincide a 6 decimales); `consumo_per_capita_kg` confirmado como derivado (`consumo_interno_kg / población`, la población implícita año a año forma una curva perfectamente suave 40,6M→47,7M que calza con la demografía real de Argentina); `precios.precio_hoja_verde_ars`/`precio_canchada_ars` — fuente encontrada y validada: resoluciones "Valores de la Materia Prima" del Directorio INYM o SAGyP por falta de unanimidad (Ley 25.564/Decreto 1240/02), publicadas en `inym.org.ar/tramites/normativa/` y `noticias/precio/`, 3 meses (ene-mar 2024) coinciden exacto contra la Resolución 406/2023 SAGyP — ver `docs/fuentes_precios_materia_prima.md`. **Hallazgo importante en el camino**: el mecanismo de precio de referencia fue discontinuado por el INYM el 31/03/2026 (Decreto 812, desregulación) — no esperar valores nuevos después de esa fecha, no es un bug del ETL si `ym.precios` no tiene datos posteriores.
- [x] **`superficie_productores` — investigación cerrada 2026-07-16** (`docs/auditoria_datos.md` §7.11): se encontró la fuente real del INYM ("Superficie Cultivada por Departamento", 2016-2025, PDF), pero revisó lo dado por válido en julio en vez de confirmarlo. El "177.533 ha congelado 2020-2024" solo coincide con el real de 2020 (174.820,1 ha) — el real sigue creciendo cada año después (231.352,4 ha en 2025). Anulado `superficie_ha` 2021-2024 (migración `008_superficie_congelada_2026-07-16.sql`), se conserva 2020. La columna `ciudad` no corresponde a ningún `departamento` real del INYM (comparado 1 a 1: factores de 0,44x a 5,2x sin patrón consistente) — documentada como no verificable por partida, no reemplazable (no hay fuente real con histórico 2010-2019 a ese nivel). Además, los 8 PDFs de esta publicación tienen una cláusula de confidencialidad/uso exclusivo INYM que los PDFs mensuales de Avance de Cosecha ya usados NO tienen — no se cargó nada de esta fuente a la plataforma pública por esa restricción, solo se usó para la comparación. `productores` por ciudad queda sin fuente citable (no existe informe histórico equivalente), orden de magnitud nacional razonable (2024: 9.334 nuestro vs. 9.709 real de prensa, Δ 3,9%) pero sin cambios en la DB.
...
- [x] **ETL de exportaciones reales por destino, mensual — COMPLETO 2026-07-12** (ver `docs/fuentes_exportaciones_indec.md`): `backend/etl/etl_indec_comex_exportaciones.py` contra INDEC Comercio Exterior (`comexbe.indec.gob.ar/public-api/search`), NCM `09030010`/`09030090`, mensual por país, 2002-2026. Tabla `ym.exportaciones_indec` (migración 004), 6.094 filas cargadas, 1.425 confidenciales (23,4%, cargadas como NULL). Validado: 55.633.560 kg reales de 57.980.911 kg oficiales INYM 2025 (96% cobertura). Gotcha real encontrado y resuelto: INDEC reporta "Chile" y "Punta Arenas (Chile)" como país separado con el mismo ISO2 — el ETL los agrega antes de insertar o rompe la PK. Endpoint `GET /exportaciones/indec` nuevo. Frontend de Exportaciones reescrito: chart mensual, distribución por destino y mapa de flujos ya usan este dato real (antes mostraban "sin desglose mensual real todavía")
...
- [x] GET `/predicciones?modelo=` — resultados pre-calculados de ML, sin entrenar en request (2026-07-18, `ym.ml_predicciones`, filtros `dimension`/`es_pronostico`)
...
**Estado: v1 completa e integrada a `/predicciones` para los 3 modelos (2026-07-18)**

### Modelo 1 — Producción por zona (título corregido, ver nota abajo)
**Estado real 2026-07-17: v1 completa (baseline), ver `docs/modelo1_produccion_zona.md`.**
...
- ⚠️ **"Por departamento" del título es aspiracional, no real — corregido 2026-07-17**: no existe producción real en kg a nivel departamento (mismo hallazgo que "Producción por ciudad" en julio, el INYM no lo publica a ese nivel). El target real es por **zona** (6 unidades, `ym.inym_hoja_verde_zona`, 2012-presente, real). Verificado que los 19 departamentos encajan 100% dentro de una sola zona cada uno (sin partirse) -- el Modelo 1 predice producción por **zona**, con NDVI/clima por departamento agregados hacia arriba. Confirmado con el usuario (eligió "modelo por zona" en vez de prorratear producción a departamento, que hubiera sido una estimación no un dato real).
- ✅ **Clima por zona — CARGADO 2026-07-17**: `ym.clima_zona_mensual` (migración `010`), `backend/etl/etl_nasa_power_zona.py`. `clima_mensual` (por ciudad) solo cubre 4 de las 6 zonas -- Noreste y Oeste no tenían ninguna "ciudad productora" adentro. Reconsultado NASA POWER en el centroide de cada zona ponderado por superficie cultivada real (`sup_ym`), no el centroide geométrico del polígono completo (que en Corrientes en particular incluye mucha superficie no yerbatera y hubiera sesgado el punto). 1.152 filas (6 zonas x 192 meses, 2010-2025), auditado (T6=192, siempre el mismo par Noreste-Noroeste, misma celda MERRA-2 ~50km, no fabricación).
- ✅ **Baseline SARIMA por zona — COMPLETO 2026-07-17** (`backend/ml/`, detalle completo en `docs/modelo1_produccion_zona.md`). Panel zona x mes armado (`build_panel_modelo1.py`), baseline estacional puro con walk-forward real (`modelo1_baseline.py`): MAE 20-30% del promedio histórico por zona. **NDVI/clima como exógena probado dos veces y en ambas empeora o no mejora el modelo** (`modelo1_exog_arranque.py`, `modelo1_exog_comparacion.py` -- el baseline sin exógena gana o empata en 5/6 zonas incluso en una prueba de 9 años). Investigado por qué Centro anda peor (arranque de temporada real y consecutivamente tardío en 2025/2026, no un bug) y una hipótesis regulatoria con evidencia parcial (desregulación yerbatera 2023-2026, eliminación de la veda de cosecha oct-nov por Resolución 2/2026 INYM -- Corrientes oct-2025 dio 2,37M kg, sin precedente en 14 años de datos). **v1 = baseline sin exógenas, es lo más simple que superó a todo lo probado.** ✅ **Integrado a `/predicciones` (2026-07-18)** — `backend/ml/scoring_modelo1.py`, ver §8 de `docs/modelo1_produccion_zona.md`.

### Modelo 2 — Consumo interno nacional
**Estado real 2026-07-17: v1 completa (baseline), ver `docs/modelo2_consumo_interno.md`.**
...
- OJO: ~~consumo per cápita parece tener variación inter-anual pero poca intra-anual~~ **CONFIRMADO 2026-07-17**: es un dato anual publicado con cadencia mensual, no sirve de target mensual. Target real usado: `ym.inym_salida_molino` (destino='interno', real, mensual, 2008-presente).
- ✅ **RIPTE (salario real) — CARGADO 2026-07-17**: `ym.indec_series`, serie `158.1_REPTE_0_0_5` (INDEC/Trabajo, real, mensual 1994-2026), agregado a `etl_indec_series.py`. Elegido sobre el "Índice de Salarios" general (solo arranca 2016-10) porque cubre todo el rango necesario.
- ✅ **Baseline SARIMA — COMPLETO 2026-07-17** (`backend/ml/build_panel_modelo2.py` + `modelo2_baseline.py`): estacionalidad mucho más débil que Producción (~20% pico-valle vs. casi-cero a picos extremos), consistente con bien de consumo cotidiano. Baseline solo: **6,3% MAPE** (60 meses walk-forward), bastante mejor que el 20-25% del Modelo 1. **Exógenas (precio relativo + salario real) no mejoran el modelo** (mismo patrón que Modelo 1) -- comparación directa en el mismo rango: baseline 7,6% vs. con exógenas 8,0%. Peor tramo del baseline: marzo-junio 2024, coincide con la recesión real de ese año (no investigado más a fondo). **v1 = baseline sin exógenas.** ✅ **Integrado a `/predicciones` (2026-07-18)** — `backend/ml/scoring_modelo2.py`, ver §7 de `docs/modelo2_consumo_interno.md`.

### Modelo 3 — Exportaciones
**Estado real 2026-07-18: v1 completa (regresión gravitacional), ver `docs/modelo3_exportaciones_gravitacional.md`.**
...
- ✅ **PBI + tipo de cambio — CARGADOS 2026-07-18**: `ym.pbi_pais_anual` (migración `011`, `etl_pbi_paises.py`, Banco Mundial `NY.GDP.MKTP.CD`, 20 países, 2011-2025 -- Siria sin dato 2023-2025, guerra civil, hueco real) y `ym.tipo_cambio_anual` (`etl_tipo_cambio.py`, ArgentinaDatos dólar oficial, 2011-2026, mejor cobertura que el BCRA REM ya cargado). Distancia calculada con las coordenadas ya cargadas en `frontend/lib/paises-destino.ts` (haversine desde Buenos Aires), sin fuente nueva.
- ✅ **Regresión gravitacional — COMPLETA 2026-07-18** (`backend/ml/build_panel_modelo3.py` + `modelo3_gravitacional.py`): panel país x año, restringido a los 20 destinos con volumen real y consistente (los otros 67 de los 87 reales son embarques sueltos/ruido, Siria+Chile+Brasil ya son 86,3% del total). Log-log OLS: PBI (+0,335, p<0,001) y distancia (-0,859, p<0,001) con signo correcto y significativos, **dummy diáspora (+4,789, p<0,001) → exp(4,79) ≈ 120x más volumen -- cuantifica el "supuesto de negocio crítico" con estadística real**. Tipo de cambio no significativo (mismo patrón que clima/NDVI en Modelo 1 y salario/precio relativo en Modelo 2 -- tercera vez que una exógena macro no aporta). R²=0,423. Walk-forward por año da 145% de MAPE (predicción a nivel de país individual es floja, esperable para un panel chico -- el modelo sirve más para explicar el patrón estructural que para pronosticar el volumen exacto). **v1 = regresión log-log con las 4 variables, sin PPML todavía.** ✅ **Integrado a `/predicciones` (2026-07-18)** — `backend/ml/scoring_modelo3.py` (ajustado-vs-real + proyección año próximo con supuesto de PBI/tipo de cambio congelados), ver §8 de `docs/modelo3_exportaciones_gravitacional.md`.
...
- [x] Guardar predicciones + intervalos de confianza en DB para que el backend solo lea (`ym.ml_predicciones`, migración `012`, 2026-07-18)
...
- [x] **Tab ML/Predicciones** (2026-07-18): reemplaza el placeholder "Coming Soon" — 3 tabs (`?modelo=1|2|3`), `ForecastBandChart` (histórico real + pronóstico con banda de IC) y `ReliabilityBadge` por modelo/zona/país citando el error real de backtest, nunca solo el número lindo. Ver Fase 5 más abajo y `frontend/app/predicciones/page.tsx`
